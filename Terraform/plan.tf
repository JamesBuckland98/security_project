...
resource "openstack_compute_secgroup_v2" "Jenkins_security_group" {
  name        = "${var.security_name2}"
  description = "${var.security_description}"
  rule {
    from_port   = 22
    to_port     = 22
...
resource "openstack_compute_instance_v2" "instance_2" {
  ...
  image_name      = "${var.image1}"
  flavor_name     = "${var.flavor}"
  security_groups = ["${openstack_compute_secgroup_v2.Jenkins_security_group.name}"]
  key_pair        = "${var.keypair}"
  user_data       = "${file("${var.server3_script}")}"
}
